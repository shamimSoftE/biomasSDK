...
class Customer_model extends CI_Model
{
	...
	public function get_brunch_wise_customer_all_info(){
		...
		$this->db->where('tbl_customer.branch_id',$this->BRANCHid)->where('tbl_customer.status', 'a');
	...
	public function get_customer_name_code_brunch_wise(){
		$res = $this->db->select('Customer_SlNo, Customer_Name, Customer_Code')->where('branch_id',$this->BRANCHid)->where('status', 'a')
	...
	{
		$res = $this->db->where('CPayment_customerID', $cus_id)->where('status','a')->get('tbl_customer_payment')->row();
		 return $res;
	}
}
