...
class Model_Table extends CI_Model
{
    ...
    public function generateFalseChalanInvoice()
    {
        $invoice = 'CH-' . date('Y') . "00001";
        $year = date('Y');
        $chalans = $this->db->query("select * from tbl_chalan_master ch where ch.SaleMaster_InvoiceNo like 'CH-$year%'");
        if ($chalans->num_rows() != 0) {
            $newChalanId = $chalans->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000');
            $invoice = 'Q-' . date('Y') . (strlen($newChalanId) > count($zeros) ? $newChalanId : $zeros[count($zeros) - strlen($newChalanId)] . $newChalanId);
        }
        return $invoice;
    }
    ...
    public function generatelcPurchaseInvoice()
    {
        $branchId = $this->session->userdata('BRANCHid');
        $branchNo = strlen($branchId) < 10 ? '0' . $branchId : $branchId;
        $invoice = date('y') . $branchNo . "00001";
        $year = date('y');
        $purchases = $this->db->query("select * from tbl_lcpurchasemaster pm where pm.PurchaseMaster_InvoiceNo like '$year%' and branch_id = ?", $branchId);
        if ($purchases->num_rows() != 0) {
            $newPurchaseId = $purchases->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000');
            $invoice = date('y') . $branchNo . (strlen($newPurchaseId) > count($zeros) ? $newPurchaseId : $zeros[count($zeros) - strlen($newPurchaseId)] . $newPurchaseId);
    ...
    public function generateSubAccountCode()
    {
        ...
        $lastRow = $this->db->query("select * from tbl_sub_account order by id desc limit 1");
        if ($lastRow->num_rows() != 0) {
            $newCode = $lastRow->row()->id + 1;
            $zeros = array('0', '00', '000');
            $code = 'SA' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }
        return $code;
    }
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            /* Received */
            (
                select ifnull(sum(sm.cash_paid), 0) from tbl_salesmaster sm
            ...
            (
                ...
                and cs.payment_type = 'Cash'
                and cs.status = 'a'
                and cs.branch_id= " . $this->session->userdata('BRANCHid') . "
                " . ($date == null ? "" : " and cs.transaction_date < '$date'") . "
            ) as total_conveyance_receive,
            ...
            (
                select ifnull(sum(cs.conveyance), 0) from tbl_conveyance_salary cs
                where cs.transaction_type = 'payment'
                and cs.payment_type = 'Cash'
                and cs.status = 'a'
                and cs.branch_id= " . $this->session->userdata('BRANCHid') . "
                " . ($date == null ? "" : " and cs.transaction_date < '$date'") . "
            ) as total_conveyance_payment,

            /* total */
            (
                select received_sales + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset + total_conveyance_receive
            ) as total_in,
            (
                select paid_purchase + paid_customer  + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset + total_conveyance_payment
            ) as total_out,
            (
    ...
    public function getBankTransactionSummary($accountId = null, $date = null)
    {
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select ifnull(sum(ct.Out_Amount), 0) from tbl_cashtransaction ct
                    where ct.bank_account_id = ba.account_id
                    and ct.status = 'a'
                    and ct.Tr_Type = 'Out Cash'
                    and ct.payment_type = 'Bank'
                    and ct.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and ct.Tr_date < '$date'") . "
                ) as total_paid_from_cash_transaction,
                ...
                (
                    ...
                    and ct.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and ct.Tr_date < '$date'") . "
                ) as total_receive_from_cash_transaction,

                (
                    select ifnull(sum(lt.amount), 0) from tbl_loan_transactions lt
                    where lt.bank_account_id = ba.account_id
                    and lt.status = 1
                    and lt.transaction_type = 'Payment'
                    and lt.payment_type = 'Bank'
                    and lt.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and lt.transaction_date < '$date'") . "
                ) as total_paid_from_loan_transaction,
                (
                    select ifnull(sum(ch.check_amount), 0) from tbl_cheque ch
                    where ch.account_id = ba.account_id
                    and ch.status = 'a'
                    and ch.check_status = 'Pa'
                    and ch.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and ch.date < '$date'") . "
                ) as total_paid_from_cheque,
                ...
                (
                    ...
                    and lt.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and lt.transaction_date < '$date'") . "
                ) as total_receive_from_loan_transaction,

                (
                    select ifnull(sum(bt.amount), 0) from tbl_investment_transactions bt
                    where bt.transaction_type = 'Receive'
                    and bt.bank_account_id = ba.account_id
                    and bt.status = 1
                    and bt.payment_type = 'Bank'
                    and bt.branch_id= " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and bt.transaction_date < '$date'") . "
                ) as total_invest_received,
                ...
                (
                    select ifnull(sum(ftr.amount), 0) from tbl_fdr_transactions ftr
                    where ftr.transaction_type = 'deposit'
                    and ftr.bank_account_id = ba.account_id
                    and ftr.status = 1
                    and ftr.branch_id= " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and ftr.transaction_date < '$date'") . "
                ) as total_fdr_paid,
                (
                    select ifnull(sum(ftr.amount), 0) from tbl_fdr_transactions ftr
                    where ftr.transaction_type = 'withdraw'
                    and ftr.bank_account_id = ba.account_id
                    and ftr.status = 1
                    and ftr.branch_id= " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and ftr.transaction_date < '$date'") . "
                ) as total_fdr_receive,
                (
                    select ifnull(sum(cs.conveyance), 0) from tbl_conveyance_salary cs
                    where cs.transaction_type = 'payment'
                    and cs.account_id = ba.account_id
                    and cs.payment_type = 'Bank'
                    and cs.status = 'a'
                    and cs.branch_id= " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and cs.transaction_date < '$date'") . "
                ) as total_conveyance_paid,
                (
                    select ifnull(sum(cs.conveyance), 0) from tbl_conveyance_salary cs
                    where cs.transaction_type = 'receive'
                    and cs.account_id = ba.account_id
                ...
                (
                    select (ba.initial_balance + total_deposit + total_received_from_customer + total_received_from_supplier + total_receive_from_cash_transaction + total_receive_from_loan_transaction + total_invest_received + total_fdr_receive + total_conveyance_receive) - (total_withdraw + total_paid_to_customer + total_paid_to_supplier + total_paid_from_cash_transaction + total_paid_from_loan_transaction + total_paid_from_cheque + total_invest_paid + total_fdr_paid + total_conveyance_paid)
                ) as balance
    ...
    public function getFDRTransactionSummary($accountId = null, $date = null)
    {
        $loanTransactionSummary = $this->db->query("
            select 
                fac.*,
                (
                    select ifnull(sum(lt.amount), 0) from tbl_fdr_transactions lt
                    where lt.fdr_account_id = fac.fdr_account_id
                ...
                (
                    select ifnull(sum(lt.amount), 0) from tbl_fdr_transactions lt
                    where lt.fdr_account_id = fac.fdr_account_id
                    and lt.transaction_type = 'withdraw'
                    and lt.status = 1
                    and lt.branch_id = " . $this->session->userdata('BRANCHid') . "
                    " . ($date == null ? "" : " and lt.transaction_date < '$date'") . "
                ) as total_received,
                ...
                (
                    select (fac.initial_balance + total_payment) - (total_received)
                ) as balance

            from tbl_fdr_accounts fac
            where fac.branch_id = " . $this->session->userdata('BRANCHid') . "
            " . ($accountId == null ? "" : " and fac.account_id = '$accountId'") . "
        ")->result();
        return $loanTransactionSummary;
    }
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    ci.*,
                    (select (ci.purchase_quantity + ci.lc_purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity) - (ci.sales_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
    ...
    public function productStock($productId)
    {
        ...
        if ($stockCount != 0) {
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->lc_purchase_quantity+ $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity)
