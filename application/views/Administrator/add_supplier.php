						...
						<tr>
							...
							<td>
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									<i class="btnEdit fa fa-pencil" @click="editSupplier(row)"></i>
									<i class="btnDelete fa fa-trash" @click="deleteSupplier(row.Supplier_SlNo)"></i>
								<?php } ?>
							</td>
						</tr>
					</template>
...
<script>
	...
	new Vue({
		...
		data() {
			return {
				supplier: {
					Supplier_SlNo: '<?php echo $supplierId; ?>',
		...
		created() {
			...
			// 	this.editSupplier(this.supplier.Supplier_SlNo);
			// }else{
				this.getSuppliers();
			// }
		},
		methods: {
			...
			// editSup(row){
			// 	window.open('/supplier/' + row.Supplier_SlNo, '_blank');
			// },

			// editSupplier(id) {
			// 	axios.post('/get_suppliers', {supplierId: id}).then(res => {
