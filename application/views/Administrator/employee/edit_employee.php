						...
						<div class="col-md-6" style="display: flex;align-items:center;">
							<div style="width: 100%;">
								<select class="chosen-select form-control" name="em_reporting_boss_id" id="em_reporting_boss_id" data-placeholder="Choose a Depertment...">
									...
									<?php
									$dquery = $this->db->query("SELECT e.*, concat(e.Employee_Name, ' - ', e.Employee_ID) as display_name FROM tbl_employee e order by e.Employee_Name asc ");
									$drow = $dquery->result();
									foreach ($drow as $drow) { ?>
										<option value="<?php echo $drow->Employee_SlNo; ?>" <?= $employee->em_reporting_boss_id == $drow->Employee_SlNo ? 'selected' : ''; ?>><?php echo $drow->display_name; ?></option>
									<?php } ?>
								</select>
							</div>
						</div>
