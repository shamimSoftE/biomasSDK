...
<div id="employeeSalary">
	<div class="row" style="margin:0px;">
		<fieldset class="scheduler-border scheduler-search">
			<legend class="scheduler-border">Salary Payment</legend>
					...
					<form class="form-inline" @submit.prevent="getEmployees">
						...
						<div class="form-group">
							...
							<div class="col-sm-1" style="padding: 0;">
								<a href="<?= base_url('month') ?>" class="btn btn-xs btn-danger" style="height: 25px; border: 0; width: 27px;margin-left: -11px;margin-top: -2px;" target="_blank" title="Add New Month"><i class="fa fa-plus" aria-hidden="true" style="margin-top: 5px;"></i></a>
							</div>
						</div>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr v-for="(employee, i) in employees" v-bind:style="{background: employee.net_payable != employee.payment ? 'orange' : ''}">
							...
							<td>
								<select 
									@change="paymentTypeChange(employee)"
									v-model="employee.payment_type"
									class="form-control" style="padding: 1px;">
									<option value="cash">Cash</option>
									<option value="bank">Bank</option>
								</select>
								<v-select v-bind:options="accounts" style="display:none;" 
									v-bind:style="{display: employee.is_bank == true ? '' : 'none'}"
									v-model="employee.selectBank" label="display_name" placeholder="Select account" ></v-select>
							</td>
							...
							<!-- <input style="width: 100px;height: 20px; text-align:center;" type="number" v-model="employee.advance" v-on:input="calculateNetPayable(employee)"> -->
							<td> {{ employee.advance }}	</td>
							<!-- <td>{{ employee.conveyance }}</td> -->
							<td style="text-align: center;">{{employee.net_payable}}</td>
							<td><input style="width: 100px;height: 20px; text-align:center;" type="number" v-model="employee.payment" v-on:input="checkPayment(employee); calculateNetPayable(employee);"></td>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			{
				data.is_bank = data.payment_type == 'cash' ? false : true;
			},
			calculateNetPayable(employee) {
				// let payable = ((parseFloat(employee.salary) + parseFloat(employee.benefit)) - (parseFloat(employee.deduction) + parseFloat(employee.advance) + parseFloat(employee.conveyance))).toFixed(2);
				let payable = ((parseFloat(employee.salary) + parseFloat(employee.benefit)) - (parseFloat(employee.deduction) + parseFloat(employee.advance) )).toFixed(2);
			...
			async getEmployees() {
				...
				if (this.payment) {
					await axios.post('/get_payments/', {
						...
						let employees =  payment.details;
						employees.map(employee=> {
							employee.is_bank = employee.payment_type == "bank" ? true : false;
							employee.selectBank = {
								account_id:employee.account_id,
								account_name:employee.account_name,
								display_name: employee.account_name +'-' + employee.account_number + '-'+ employee.bank_name
							};
							return employee;
						});
					...
					await axios.post('/get_employees', {
						month_id: month_id,
					}).then(res => {
						...
						employees.map(employee => {
							...
							employee.comment = '';
							employee.payment_type = 'cash';
							employee.is_bank = false;
							employee.selectBank = null;
							return employee;
						});
