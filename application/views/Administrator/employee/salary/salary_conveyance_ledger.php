...
<div id="paymentsConveyance">
	<div class="row" style="margin:0;">
		<fieldset class="scheduler-border scheduler-search">
			<legend class="scheduler-border">Salary Conveyance Ledger</legend>
	...
	<div class="row" style="display:none;" v-bind:style="{display: payments.length > 0 ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table class="record-table table table-bordered table-hover" v-if="(searchTypesForRecord.includes(searchType)) " style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) ? '' : 'none'}">
					<thead>
						<tr>
							<th>Employee ID</th>
							<th>Employee Name</th>
					...
					<tbody>
						...
						<tr v-for="(employee, sl) in payments">
							<td>{{ employee.Employee_ID }}</td>
							<td>{{ employee.Employee_Name }}</td>
							<td style="text-align:right;">{{ employee.month_name }}</td>
							<td style="text-align:center;">{{ employee.transaction_date }}</td>
							<td style="text-align:right;text-transform: capitalize;">
								{{ employee.transaction_type }}
							</td>
							<td style="text-align:right;">{{ employee.conveyance }}</td>
							<td>{{ employee.note }}</td>
						</tr>
					...
					<table class="record-table">
						...
						<tbody>
							<tr v-for="payment in payments">
								...
								<td style="text-transform: capitalize;">{{ payment.transaction_type }}</td>
								<td style="text-align:right;">{{ payment.conveyance }}</td>
								<td>{{ payment.note }}</td>
							</tr>
						</tbody>
						<tfoot>
							<tr style="font-weight:bold;">
								<td colspan="3" style="text-align:right;">Total=</td>
								<td style="text-align:right;">{{ payments.reduce((prev, curr) => { return prev + parseFloat(curr.conveyance)}, 0) | decimal }}</td>
								<td></td>
							</tr>
						</tfoot>
					</table>
		...
		methods: {
			...
			getPaymentsRecord() {
				...
				let filter = {
					...
					employee_id: this.selectedEmployee == null ? null : this.selectedEmployee.Employee_SlNo
				}
				...
				let url = '/get_conveyance_details';
