...
<div id="purchaseRecord">
	<div class="row" style="margin:0;">
		<fieldset class="scheduler-border scheduler-search">
			<legend class="scheduler-border">Search Purchase Record</legend>
						...
						<template v-for="purchase in purchases">
							<tr>
								...
								<td style="text-align:center;">
									<span v-if="purchase.is_lc_purchase == 1" style="color: red; font-weight: bold;">LC</span>
								...
								<td style="text-align:center;">
									...
									<?php if ($this->session->userdata('accountType') != 'u') { ?>
										<!-- <a href="javascript:" v-if="purchase.status == 'p'" title="Edit Purchase" @click="changeStatus(purchase.PurchaseMaster_SlNo)"><i class="fa fa-edit"></i></a> -->
										<!-- <a href="" title="Delete Purchase" v-if="purchase.status == 'p'" @click.prevent="deletePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-trash"></i></a> -->
					...
					<tbody>
						<tr v-for="purchase in purchases">
							...
							<td style="text-align:center;">
								<a href="" title="Purchase Invoice" v-bind:href="`/purchase_invoice_print/${purchase.PurchaseMaster_SlNo}`" target="_blank"><i class="fa fa-file-text"></i></a>
								<?php if ($this->session->userdata('accountType') != 'u') { ?>
									<!-- <a href="javascript:" title="Edit Purchase" v-if="purchase.status == 'p'" @click="changeStatus(purchase.PurchaseMaster_SlNo)"><i class="fa fa-edit"></i></a>
									<a href="" title="Delete Purchase" v-if="purchase.status == 'p'" @click.prevent="deletePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-trash"></i></a> -->
									<a href="javascript:" title="Edit Purchase" @click="checkReturnAndEdit(purchase)"><i class="fa fa-edit"></i></a>
									<a href="" title="Delete Purchase" @click.prevent="deletePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-trash"></i></a>
								<?php } ?>
...
<script>
	...
	new Vue({
		...
		methods: {
			changeStatus(purchaseId) {
				let approveConf = confirm('Do you want to change the status?');
				if (approveConf == false) {
					return;
				}

				axios.post('/change_purchase_status', {
			...
			sorting() {
				if (this.sortRecord === 'asc') {
					this.purchases.sort((a, b) => (a.Supplier_Name || '').localeCompare(b.Supplier_Name || ''));
				} else if (this.sortRecord === 'desc') {
					this.purchases.sort((a, b) => (b.Supplier_Name || '').localeCompare(a.Supplier_Name || ''));
				}
			},
			checkReturnAndEdit(purchase) {
				axios.get('/check_purchase_return/' + purchase.PurchaseMaster_InvoiceNo).then(res => {
					if (res.data.found) {
						...
						window.open('/purchase/' + purchase.PurchaseMaster_SlNo, '_blank');
					}
				})
			},
			onChangeSearchType() {
