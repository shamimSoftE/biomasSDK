					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:center;">{{ sale.saleDetails[0].stock }}</td>
								<td style="text-align:right;">{{ sale.saleDetails[0].SaleDetails_TotalAmount | decimal }}</td>
								<td style="text-align:center;">
										<a href="" title="Order Invoice" v-bind:href="`/order_invoice_print/${sale.SaleMaster_SlNo}`" target="_blank"><i class="fa fa-file"></i></a>
										
										<?php if ($this->session->userdata('accountType') != 'u') { ?>
											<a href="" title="Sale" @click.prevent="orderToSale(sale.SaleMaster_SlNo)"><i class="fa fa-arrow-circle-right" style="color: red;"></i></a>
				...
				<table class="table table-hover record-table table-bordered" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;">{{ sale.cash_paid | decimal }}</td>
							<td style="text-align:right;">{{ sale.bank_paid | decimal }}</td>
						...
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.cash_paid)}, 0) | decimal }}</td>
							<td style="text-align:right;">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.bank_paid)}, 0) | decimal }}</td>
	...
	new Vue({
		...
		methods: {
			checkReturnAndEdit(sale) {
						location.replace('/orders/' + sale.SaleMaster_SlNo);
			},
			onChangeSearchType() {
				this.sales = [];
