						...
						<form v-on:submit.prevent="addToCart">
							...
							<div class="form-group clearfix">
								...
								<div class="col-xs-9" style="display: flex;align-items:center;margin-bottom:5px;">
									<div style="width: 88%;">
										<v-select v-bind:options="categories" style="margin:0;" v-model="selectedCategory" @input="categoryOnChange" label="ProductCategory_Name"></v-select>
									</div>
							...
							<div class="form-group">
								...
								<div class="col-xs-9">
									<input type="number" id="salesRate" placeholder="Rate" ref="salesRate" step="0.01" class="form-control" v-model="selectedProduct.Product_SellingPrice" v-on:input="productTotal" />
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-xs-9">
									<input type="text" placeholder="note" class="form-control" v-model="productNote" />
								</div>
							</div>
				...
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
							...
							<tr>
								<td colspan="9"><textarea class="form-control" style="font-size:13px;margin-top:3px;" placeholder="Note" v-model="sales.note"></textarea></td>
...
<script>
	...
	new Vue({
		...
		async created() {
		 	...
		 	// const locationInfo =	navigator.geolocation.getCurrentPosition(position => console.log(position));
		...
		methods: {
			...
			getProducts() {
				...
				let categoryId = this.selectedCategory != null ? this.selectedCategory.ProductCategory_SlNo : '';
			...
			productTotal() {
				let productTotal = (parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct.Product_SellingPrice)).toFixed(2);
				...
				if (this.productDiscountAmount != undefined) {
			...
			quantityRateChange() {
				...
				this.cart = this.cart.map(item => {
					total = parseFloat(parseFloat(item.salesRate) * parseFloat(item.quantity)).toFixed(2);
					// product wise discount
			...
			async getSales() {
				await axios.post('/get_sales', {
					salesId: this.sales.salesId,sale_type:'sale',status:'a'
				}).then(res => {
					let r = res.data;
					let sales = r.sales[0];
					...
					this.sales.employeeId = sales.employee_id;
