<style>
	...
	.table > tbody > tr > td { text-align: start; }
</style>
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:center;">{{ sale.saleDetails[0].SaleDetails_TotalQuantity }} {{ sale.saleDetails[0]?.Unit_Name }}</td>
							...
							<tr v-for="(product, sl) in sale.saleDetails.slice(1)">
								...
								<td style="text-align:center;">{{ product.SaleDetails_TotalQuantity }} {{ product.Unit_Name }}</td>
				...
				<table class="table table-hover record-table table-bordered" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					<thead>
						<tr>
							...
							<th style="background-color: green;color:white">Paid</th>
							<th style="background-color: red;color:white">Due</th>
							<th>Note</th>
							<th>Action</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;background-color: blue;color:white">{{ sale.SaleMaster_TotalSaleAmount | decimal }}</td>
							<td style="text-align:right;background-color: green;color:white">{{ sale.SaleMaster_PaidAmount | decimal }}</td>
							<td style="text-align:right;background-color: red;color:white">{{ sale.SaleMaster_DueAmount | decimal }}</td>
						...
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right; background-color: blue;color:white">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_TotalSaleAmount)}, 0) | decimal }}</td>
							<td style="text-align:right;background-color: green;color:white">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_PaidAmount)}, 0) | decimal }}</td>
							<td style="text-align:right;background-color: red;color:white">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.SaleMaster_DueAmount)}, 0) | decimal }}</td>
							<td></td>
							<td></td>
						</tr>
					</tbody>
							...
							<template v-for="sale in sales">
								...
								<tr>
									<td colspan="4" style="text-align: end;">
										<strong> Total: 
										{{ sale.products.reduce((prev, curr) => {return prev + parseFloat(curr.amount)}, 0) }}
										</strong>
									</td>
								</tr>
							</template>
		...
		methods: {
			sorting() {
				if (this.sortRecord === 'asc') {
					this.sales.sort((a, b) => (a.Customer_Name || '').localeCompare(b.Customer_Name || ''));
				} else if (this.sortRecord === 'desc') {
					this.sales.sort((a, b) => (b.Customer_Name || '').localeCompare(a.Customer_Name || ''));
				}
			},
			checkReturnAndEdit(sale) {
				axios.get('/check_sale_return/' + sale.SaleMaster_InvoiceNo).then(res => {
					if (res.data.found) {
						...
						window.open('/sales/' + sale.SaleMaster_SlNo, '_blank');
					}
				})
			},
			onChangeSearchType() {
										...
										products: _.chain(sale)
											...
											.map(product => {
												return {
													...
													amount: _.sumBy(product, item => Number(item.SaleDetails_TotalAmount))
												}
											})
											.value()
									}
